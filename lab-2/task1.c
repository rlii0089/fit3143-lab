 ...
 */
long *find_local_primes(long start_index, long stride, long end_index, long *local_prime_count) {
    long capacity = 1024;
    long count = 0;

    long *local_primes = malloc((size_t)capacity * sizeof(long));
    ...
    for (long index = start_index; index < end_index; index += stride) {
        long number = candidate_to_number(index);

        if (is_prime_number(number)) {
            if (count == capacity) {
                long new_capacity = capacity * 2;

                long *resized = realloc(local_primes, (size_t)new_capacity * sizeof(long));
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        if (argc < 2 || argc > 3) {
            fprintf(stderr, "Usage: %s <n> [block|cyclic|weighted]\n", argv[0]);
        ...
        if (n >= 0 && argc == 3) {
            ...
            else if (strcmp(argv[2], "cyclic") == 0) {
    ...
    long *local_primes = find_local_primes(start_index, stride, end_index, &local_prime_count);
     ...
     */
    int local_error = (local_primes == NULL) ? 1 : 0;
